...
describe('converters/JourneyDetailConverter', () => {
  ...
  it('can handle error journey', () => {
    const json = require('./JourneyDetail.error.json')
    const output = JourneyDetailConverter(json)
    expect(output.stops.length).toBe(0)
  ...
  it('can convert dates', () => {
    ...
    const output = JourneyDetailConverter(json)
    const stop: JourneyStop = output.stops[0]

    expect(stop.name).toBe('Solrød Strand St.')
    expect(stop.routeIdx).toBe(0)
    expect(stop.departure).not.toBe(undefined)
    expect(stop.departure?.datetime.toFormat('yyyy-MM-dd HH:mm')).toBe(
      '2021-03-10 02:14'
    )
    expect(stop.departure?.rtDatetime?.toFormat('yyyy-MM-dd HH:mm')).toBe(
      '2021-03-10 02:14'
    )
  ...
  it('can convert stops to ts', () => {
    ...
    expect(stop.departure?.changed).toBe(false)
    expect(stop.arrival?.datetime.toFormat('yyyy-MM-dd HH:mm')).toBe(
      '2021-03-10 02:15'
    )
    ...
    expect(stop.arrival?.rtDatetime?.toFormat('yyyy-MM-dd HH:mm')).toBe(
      '2021-03-10 03:16'
    )
