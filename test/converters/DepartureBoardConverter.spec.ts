...
describe('converters/DepartureBoardConverter', () => {
  it('can convert json', () => {
    ...
    expect(departure.time.changed).toBe(false)
    expect(departure.time.rtTime).toBe(undefined)
    expect(departure.time.rtDate).toBe(undefined)
    expect(departure.time.rtDatetime).toBe(undefined)
    expect(departure.cancelled).toBe(false)
